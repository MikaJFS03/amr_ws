...
# class NonHumanPointCloudNode(Node):
#     def __init__(self):
#         super().__init__('unperson')
...
#         self.nonhuman_publisher = self.create_publisher(PointCloud2, '/detected_object/unperson', 10)
#         self.active_points = {}

#     def coord_callback(self, msg):
#         data = msg.data.split(',')
#         cls_id = int(data[0])
#         if cls_id != 0:  # Selain manusia
#             x, y, z = map(float, data[1:])
#             self.active_points[(x, y, z)] = time.time()  # Tambah waktu aktif

#         self.update_pointcloud()

#     def update_pointcloud(self):
#         current_time = time.time()
#         points = [p for p, t in self.active_points.items() if current_time - t < 4]

#         # Update daftar aktif
#         self.active_points = {p: t for p, t in self.active_points.items() if current_time - t < 4}
...
#         )
#         self.nonhuman_publisher.publish(pointcloud)
