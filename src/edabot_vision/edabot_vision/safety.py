...
class SafetyPointCloud(Node):
    def __init__(self):
        ...
        self.camera_info = None
        self.pointcloud_buffer = []
        self.pointcloud_lifetime = 1.0  # Lifetime in seconds
        self.get_logger().info(f'Subscribed to image topic: {self.image_topic}')
        self.get_logger().info(f'Subscribed to depth topic: {self.depth_topic}')
        self.get_logger().info(f'Subscribed to camera info topic: {self.camera_info_topic}')
    ...
    def publish_pointcloud(self, points):
        ...
        for x, y, z in points:
            transformed_point = self.transform_to_map(x, y, z)
        ...
        if len(transformed_points) > 0:
            cloud_data = np.asarray(transformed_points, dtype=np.float32).tobytes()
            pointcloud = PointCloud2(
                header=header,
                height=1,
                width=len(transformed_points),
                is_dense=True,
                is_bigendian=False,
                point_step=12,
                row_step=12 * len(transformed_points),
                fields=fields,
                data=cloud_data
            )
        else:
